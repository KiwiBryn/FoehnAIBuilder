...
/// </summary>
public sealed class RmdirTool(ILogger<RmdirTool> logger) : ITool
{
   public string Name => "directory.remove";
    ...
    public async Task<ToolExecutionResult> ExecuteAsync(string argumentsJson, CancellationToken cancellationToken = default)
    {
        ...
        {
            logger.LogWarning("Failed to parse rmdir arguments: {Arguments} ({Error})", argumentsJson, jsonError);
        ...
        logger.LogInformation("Removing directory {Path} (recursive={Recursive})", path, recursive);
        ...
        try
        {
            ...
            return ToolExecutionResult.Ok($"Removed directory \"{path}\".");
        }
        catch (IOException ex)
        {
            logger.LogWarning(ex, "Directory not empty: {Path}", path);
            return ToolExecutionResult.Fail($"Could not remove \"{path}\": {ex.Message} (pass recursive=true to delete a non-empty directory).");
        }
        catch (UnauthorizedAccessException ex)
        {
            logger.LogError(ex, "Error removing directory {Path}", path);
            return ToolExecutionResult.Fail($"Error removing \"{path}\": {ex.Message}");
        }
    }
}
